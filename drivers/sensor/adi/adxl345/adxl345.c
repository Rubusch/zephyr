			   ...
			   uint16_t *fifo_entries)
{
	...
	ret = adxl345_reg_read(dev, ADXL345_REG_INT_SOURCE, buf, length);

	*status1 = buf[0];
	ret = adxl345_reg_read(dev, ADXL345_REG_FIFO_STATUS, buf+1, length);
	if (fifo_entries) {
...
{
	return adxl345_reg_write_mask(dev, ADXL345_REG_POWER_CTL,
					   ADXL345_POWER_CTL_MODE_MSK,
				...
				enum sensor_channel chan)
{
	...
	rc = adxl345_reg_read_byte(dev, ADXL345_REG_FIFO_STATUS, &samples_count);
	if (rc < 0) {
...
static int adxl345_init(const struct device *dev)
{
	...
	rc = adxl345_reg_write_byte(dev, ADXL345_REG_FIFO_CTL, ADXL345_FIFO_STREAM_MODE);
	if (rc < 0) {
	...
	rc = adxl345_reg_write_byte(dev, ADXL345_REG_DATA_FORMAT, ADXL345_DATA_FORMAT_RANGE_8G);
	if (rc < 0) {
		LOG_ERR("Data format set failed\n");
		return -EIO;
	}

	data->selected_range = ADXL345_DATA_FORMAT_RANGE_8G;
	...
	rc = adxl345_reg_write_byte(dev, ADXL345_REG_POWER_CTL,
				    ADXL345_POWER_CTL_MODE_MSK);
	if (rc < 0) {
	...
	rc = adxl345_interrupt_config(dev, ADXL345_REG_INT_MAP_WATERMARK_MSK);
	if (rc) {
		return rc;
	}
#endif
...
#define ADXL345_CONFIG(inst)						\
		...
		.fifo_config.fifo_samples = ADXL345_FIFO_CTL_SAMPLES_MSK, \
...
#define ADXL345_DEFINE(inst)						\
	...
	static struct adxl345_dev_data adxl345_data_##inst = {		\
		COND_CODE_1(adxl345_iodev_##inst,			\
