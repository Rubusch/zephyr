...
#define ADXL345_DATA_FORMAT_RIGHT_JUSTIFY BIT(2) /* 1: data left justified */
#define ADXL345_DATA_FORMAT_FULL_RES	BIT(3)
#define ADXL345_DATA_FORMAT_INT_INVERT	BIT(5) /* 1: interrupts active low */
...
struct adxl345_fifo_data {
	...
	uint8_t int_status; // TODO duplicate with cached int_status?
...
BUILD_ASSERT(sizeof(struct adxl345_fifo_data) % 4 == 0,
	     "struct adxl345_fifo_data should be word aligned");
...
# endif
	...
	uint8_t status; // TODO duplicate with int_status?
	uint8_t fifo_ent[1]; // TODO needed?
	uint64_t timestamp;
	struct rtio *r_cb;
	uint8_t fifo_samples; // TODO fifo samples -> fifo_entries and add fifo_size
